...
void multiply_vectors(data_T in1[N], data_T in2[N], res_T out[N]) {
    MULTIPLY_VECT_LOOP:
    #pragma unroll
    for (int i = 0; i < N; i++) {
        out[i] = in1[i] * in2[i];
...
template<class data_T, class res_T, int N>
void add_vectors(data_T in1[N], data_T in2[N], res_T out[N]) {
    ADD_VECTOR_LOOP:
    #pragma unroll
    for (int i = 0; i < N; i++) {
        out[i] = in1[i] + in2[i];
...
void simple_rnn_cell(
    ...
    multiply_W<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_in, CONFIG_T::n_out>(inputs, afterW, kernel);

    // Bias addition
    typename CONFIG_T::accum_t afterBias[CONFIG_T::n_out] hls_register;
    add_bias<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::bias_t, CONFIG_T::n_out>(afterW, afterBias, bias);

    // Hidden state
    typename CONFIG_T::accum_t hiddenCand[CONFIG_T::n_out] hls_register;
    multiply_U<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_out>(hidden_state, hiddenCand, rec_kernel);
    ...
    // Activation
    CONFIG_T::template activation<typename CONFIG_T::accum_t, data_T, typename CONFIG_T::ACT_CONFIG_T>::activation(afterAdd, hidden_state_o);
}
...
void lstm_cell(
    ...
    //-----------Gate I Calculations
    // Weight multiplication     
    multiply_W<data_T,typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_in, CONFIG_T::n_out>(inputs, i_afterW , WI);

    // Bias addition
    add_bias<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::bias_t, CONFIG_T::n_out>(i_afterW, i_afterBias, BI);

    // Hidden Candidate
    multiply_U<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_out>(hidden_state, i_hiddenCand, RWI);

    // Vector addition
    add_vectors<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, CONFIG_T::n_out>(i_afterBias, i_hiddenCand, i_afterAdd);

    // Activation
    CONFIG_T::template activation_recr<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::ACT_CONFIG_RECURRENT_T>::activation(i_afterAdd, gate_i);



    //-----------Gate F Calculations
    // Weight multiplication
    multiply_W<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_in, CONFIG_T::n_out>(inputs, f_afterW, WF);

    // Bias addition
    add_bias<typename CONFIG_T::accum_t,typename CONFIG_T::accum_t, typename CONFIG_T::bias_t, CONFIG_T::n_out>(f_afterW, f_afterBias, BF);

    // Hidden Candidate
    multiply_U<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_out>(hidden_state, f_hiddenCand, RWF);

    // Vector addition
    add_vectors<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, CONFIG_T::n_out>(f_afterBias, f_hiddenCand, f_afterAdd);

    // Activation
    CONFIG_T::template activation_recr<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::ACT_CONFIG_RECURRENT_T>::activation(f_afterAdd, gate_f);

    //-----------Gate C Calculations
    // Weight multiplication
    multiply_W<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_in, CONFIG_T::n_out>(inputs, c_afterW, WC);

    // Bias addition
    add_bias<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::bias_t, CONFIG_T::n_out>(c_afterW, c_afterBias, BC);

    // Hidden Candidate
    multiply_U<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_out>(hidden_state, c_hiddenCand, RWC);

    // Vector addition
    add_vectors<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, CONFIG_T::n_out>(c_afterBias, c_hiddenCand, c_afterAdd);

    // Activation 
    CONFIG_T::template activation<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::ACT_CONFIG_RECURRENT_T>::activation(c_afterAdd, gate_c);
    ...
    //-----------Gate O Calculations
    // Weight multiplication
    multiply_W<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_in, CONFIG_T::n_out>(inputs, o_afterW, WO);

    // Bias addition
    add_bias<typename CONFIG_T::accum_t,typename CONFIG_T::accum_t,typename CONFIG_T::bias_t, CONFIG_T::n_out>(o_afterW, o_afterBias, BO);

    // Hidden Candidate
    multiply_U<data_T, typename CONFIG_T::accum_t, typename CONFIG_T::weight_t, CONFIG_T::n_out>(hidden_state, o_hiddenCand, RWO);

    // Vector addition
    add_vectors<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, CONFIG_T::n_out>(o_afterBias, o_hiddenCand, o_afterAdd);

    // Activation 
    CONFIG_T::template activation_recr<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::ACT_CONFIG_RECURRENT_T>::activation(o_afterAdd, gate_o);
    ...
    // Vector addition
    add_vectors<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, CONFIG_T::n_out>(gate_ic, cell_act_multp, cell_act_add);

    //-----------Forget gate Calculation
    // Activation 
    CONFIG_T::template activation<typename CONFIG_T::accum_t, typename CONFIG_T::accum_t, typename CONFIG_T::ACT_CONFIG_RECURRENT_T>::activation(cell_act_add, gate_forget);
...
void lstm( 
    ...
    for (int i = 0; i < CONFIG_T::n_timesteps; i++){
        ...
        // Do LSTM
        lstm_cell<data_T, res_T, CONFIG_T>(in, hidden_state_temp, h, cell_state_temp, c, WI, WF, WC, WO, RWI, RWF, RWC, RWO, BI, BF, BC, BO);
